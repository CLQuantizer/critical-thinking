...
export const chunkText = (text: string, maxParagraphLength: number) => {
    const regex = new RegExp(
        ...
        `(?!${AVOID_AT_START})(?:(?:^|\\r?\\n\\r?\\n)(?:<p>)?${SENTENCE_PATTERN.replace(/{MAX_LENGTH}/g, String(maxParagraphLength))}(?:</p>)?(?=\\r?\\n\\r?\\n|$))` +
        "|" +
    ...
    if (matches) {
        // log how many chunks are there
        console.log(`\n\nNumber of chunks: ${matches.length}\n\n`);
        return matches;
